...
import biplist, paramiko, re, os, json, shutil, io
...
class iDeviceWorker(Worker):
	...
	if(" " in exetuable_name):
        ...
        getfilebinPath = "cd /var/mobile/Applications/%s/*.app/ && echo $(pwd)/%s" % (self.__long_hex_key, exetuable_name)
	...
	if ("\ " in binary_filename):
		...
		mv_command = 'mv %s.decrypted %s.decrypted'%(binary_filename, newbinary_filename)

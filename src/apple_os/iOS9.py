...
class iDeviceWorker(Worker):
    ...
    def setItemId_and_getPlistDict(self):
        try:
            ...
            cmd = 'cat /var/mobile/Containers/Bundle/Application/%s/iTunesMetadata.plist' % self.__long_hex_key
            ...
            find_com = 'find /var/mobile/Containers/Bundle/Application/%s/*.app' % self.__long_hex_key
            self._ssh.connect(**self._connectArgs)
    ...
    def findBin(self):
        ...
        #filetype = ['Mach-O fat file with 2 architectures']
        self._ssh.connect(**self._connectArgs)
        cmd = 'cat /var/mobile/Containers/Bundle/Application/%s/*.app/Info.plist'
        ...
        getfilebinPath = "cd /var/mobile/Containers/Bundle/Application/%s/*.app/ && echo $(pwd)/%s" % (self.__long_hex_key, exetuable_name)
    ...
    def getOriginalBin(self):
        ...
        sftp.close()
        self._ssh.close()
        print "Downloaded Original Binary File."
        return target_path
    ...
    def doDecrypted(self):
        ...
        print "This app has been decrypted."
	time.sleep(10)
        self._ssh.close()
	...
	print 'Start downloading %s.dercrypted file.' %self.binary_filename
	self._ssh.connect(**self._connectArgs)
        sftp = self._ssh.open_sftp()
        sftp.get(de_path, target_path)
        sftp.close()
        self._ssh.close()
	print '%s.dercrypted file has been downloaded.' %self.binary_filename
...
class LinuxWorker(object):
    ...
    def getPair(self, PlistDict):
        ...
        try:
            ...
            print 'ERROR:', err
	pathSplit = self.binpath.split('binary')
	newPath = pathSplit[0] + 'json' + pathSplit[1]
        pList = open('%s.plist.json' % newPath, 'w')
        pList.write(content)
        pList.close()
        print 'Json data has been done.'
